					...
					<div id="list">
						<form action="list" method="get">
							<div class="form-group text-right">
								<input type="text" name="keword" value="">
								...
								<c:forEach items="${bList}" var="boardVo" varStatus="status">
									<tr>
										...
										<td class="text-left"><a
											href="${pageContext.request.contextPath}/rbrc/read?no=${boardVo.no}&hit=1">${boardVo.title}</a></td>
										<td>${boardVo.name}</td>
										<td>${boardVo.hit}</td>
						...
						<c:if test="${!(empty authUser)}">
							<a id="btn_write"
								href="${pageContext.request.contextPath}/rbrc/writeForm">글쓰기</a>
						</c:if>
					</div>
					<!-- //list -->
				</div>
